...
def train_task_one_step(model1, num_classes, filename_csv_train, FILENAME_CSV_VALID, filename_csv_test=None,
                        ...
                        label_smoothing=0, class_weights=None,
    ...
    model_finetuning = my_transfer_learning.convert_model_transfer(model1, clsss_num=num_classes,
                        change_top=change_top, activation_function='Sigmoid',
                        freeze_feature_extractor=False)
    model_finetuning = my_transfer_learning.convert_trainable_all(model_finetuning)
    ...
    custom_loss_function = get_weighted_binary_crossentropy(class_weights)
    model_finetuning.compile(loss=custom_loss_function,
            optimizer=op_finetuning, metrics=['acc'])
    ...
    my_gen_train = My_images_generator(files=train_files, labels=train_labels,
                image_shape=input_shape, num_output=num_classes,
                multi_labels=True, imgaug_seq=imgaug_train_seq, batch_size=batch_size_train)

    my_gen_valid = My_images_generator(files=valid_files, labels=valid_labels,
                image_shape=input_shape, num_output=num_classes,
                multi_labels=True, batch_size=batch_size_valid)
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if compute_cf_train:
                ...
                my_gen_train_test = My_images_generator(files=train_files, labels=train_labels,
                                                        num_output=num_classes, multi_labels=True,
                                                        image_shape=input_shape,  batch_size=batch_size_train)
