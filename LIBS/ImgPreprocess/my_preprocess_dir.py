...
def do_preprocess_dir(source_dir, dest_dir, image_size=299, is_rop=False,
    ...
    for dir_path, subpaths, files in os.walk(source_dir, False):
        for f in files:
            ...
            preprocess_image(img_file_source, img_file_dest, image_size,
                             is_rop=is_rop, add_black_pixel_ratio=add_black_pixel_ratio)
...
def preprocess_image(img_source, img_file_dest, crop_size, add_black_pixel_ratio,
                     is_rop=False, SMALL_SIZE=2048):
    try:
    ...
    if img is not None:
        if not is_rop:
            ...
            image1 = my_preprocess.do_preprocess_rop(img, crop_size=crop_size, add_black_pixel_ratio=add_black_pixel_ratio)
...
if __name__ == '__main__':
    ...
    do_preprocess_dir(dir_original, dir_preprocess, image_size=512)
