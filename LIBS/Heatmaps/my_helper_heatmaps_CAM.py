...
def server_cam(dicts_models, model_no, img_source, pred, cam_relu=True, preprocess=True,
    ...
    input_shape = dicts_models[model_no]['input_shape']
    ...
    if isinstance(img_source, str):
        if preprocess:
            ...
            img_input = LIBS.ImgPreprocess.my_image_helper.my_gen_img_tensor(img_source,
                                                                             image_shape=input_shape)
    if isinstance(img_source, np.ndarray): # batch, h ,w, c
        assert img_source.ndim == 4, 'ndim error'
...
def server_gradcam_plusplus(dicts_models, model_no, img_source, pred, preprocess=True,
                    blend_original_image=True, base_dir_save='/tmp3'):
    input_shape = dicts_models[model_no]['input_shape']
    image_size = input_shape[0]  #square
...
#grad_cam do not use vis lib
#https://github.com/eclique/keras-gradcam/blob/master/grad_cam.py
def grad_cam(input_model, image, cls, layer_name, Width, Height):
...
def server_grad_cam(dicts_models, model_no, img_source, pred, preprocess=True,
    ...
    gradcam = grad_cam(model1, img_input, class_predict, layer_name,
                       Width=width, Height=height)

    from LIBS.ImgPreprocess.my_image_norm import input_norm_reverse
    image_original = input_norm_reverse(img_input)
